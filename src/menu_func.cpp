...
void make_flight_record(FlightData& flights){
	FlightInfo flight;
	if(flight.read()) flights.add_flight(flight);
}

void make_user_record(UserData& users){
	UserInfo user;
	if(user.read()) users.add_user(user);
}
...
void record_in_file(FlightData& flights,UserData& users){
	Recorder::save_flights(flights);
	Recorder::save_users(users);
}
